...
# 1e is like 1d, while it introduces pitch and a new architecture to
# fit spanish english code switch
# Modified by: apj2125
...
# AJ To-Do: Do we need to change this?
...
if [ $stage -le 14 ]; then
  ...
  if [ -d data/lang_chain ]; then
    if [ data/lang_chain/L.fst -nt data/lang_refined/L.fst ]; then
    ...
    cp -r data/lang_refined data/lang_chain
...
if [ $stage -le 15 ]; then
  ...
  steps/align_fmllr_lats.sh --nj 8 --cmd "$train_cmd" ${lores_train_data_dir} \
    data/lang_refined $gmm_dir $lat_dir
...
if [ $stage -le 17 ]; then
  ...
  relu-batchnorm-dropout-layer name=tdnn1 $affine_opts dim=758
  tdnnf-layer name=tdnnf2 $tdnnf_opts dim=758 bottleneck-dim=128 time-stride=1
  tdnnf-layer name=tdnnf3 $tdnnf_opts dim=758 bottleneck-dim=128 time-stride=1
  tdnnf-layer name=tdnnf4 $tdnnf_opts dim=758 bottleneck-dim=128 time-stride=1
  tdnnf-layer name=tdnnf5 $tdnnf_opts dim=758 bottleneck-dim=128 time-stride=0
  tdnnf-layer name=tdnnf6 $tdnnf_opts dim=758 bottleneck-dim=128 time-stride=3
  tdnnf-layer name=tdnnf7 $tdnnf_opts dim=758 bottleneck-dim=128 time-stride=3
  tdnnf-layer name=tdnnf8 $tdnnf_opts dim=758 bottleneck-dim=128 time-stride=3
  linear-component name=prefinal-l dim=256 $linear_opts

  prefinal-layer name=prefinal-chain input=prefinal-l $prefinal_opts big-dim=758 small-dim=256
  output-layer name=output include-log-softmax=false dim=$num_targets $output_opts

  prefinal-layer name=prefinal-xent input=prefinal-l $prefinal_opts big-dim=758 small-dim=256
...
if [ $stage -le 19 ]; then
  ...
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 data/lang_refined $dir $dir/graph
fi
...
if [ $stage -le 20 ]; then
  ...
  for dset in miami/test; do
      ...
      steps/nnet3/decode.sh --num-threads 4 --nj $decode_nj --cmd "$decode_cmd" \
         ...
         $dir/graph data/${dset}_hires $dir/decode || exit 1;
      steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" data/lang_refined data/lang_refined_rescore \
        data/${dset}_hires ${dir}/decode ${dir}/decode_rescore || exit 1
    ) || touch $dir/.error &
  done
  wait
  if [ -f $dir/.error ]; then
