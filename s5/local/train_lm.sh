...
if [ $stage -le 0 ]; then
  ...
  #head -n $num_dev_sentences < ${dir}/data/text_lm.txt | cut -d " " -f 2-  > ${dir}/data/text/dev.txt
  ...
  # get wordlist | does this need to be with the class labels or without?
  awk '{print $1}' data/local/dict/lexicon.txt | sed 's:([0-9])::g' | sort | uniq > ${dir}/data/wordlist
fi
...
if [ $stage -le 1 ]; then
  ...
  #              ${dir}/data/text_lm.txt ${order} ${lm_dir}/work ${unpruned_lm_dir}

  train_lm.py  --num-splits=8 --warm-start-ratio=20  \
               --limit-unk-history=true \
