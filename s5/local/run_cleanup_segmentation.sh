...
if [ $stage -le 1 ]; then
  ...
  steps/cleanup/clean_and_segment_data.sh --stage $cleanup_stage --nj $nj --cmd "$train_cmd" \
    $data data/lang_refined $srcdir $dir $cleaned_data
fi

if [ $stage -le 2 ]; then
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    $cleaned_data data/lang_refined $srcdir ${srcdir}_ali_${cleanup_affix}
fi

if [ $stage -le 3 ]; then
  steps/train_sat.sh --cmd "$train_cmd" \
    5000 100000 $cleaned_data data/lang_refined ${srcdir}_ali_${cleanup_affix} ${cleaned_dir}
fi

if [ $stage -le 4 ]; then
  # Test with the models trained on cleaned-up data.
  utils/mkgraph.sh data/lang_refined ${cleaned_dir} ${cleaned_dir}/graph

  steps/decode_fmllr.sh --nj $decode_nj --cmd "$decode_cmd" --num-threads $decode_num_threads \
     ${cleaned_dir}/graph data/miami/test ${cleaned_dir}/decode
  steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" data/lang_refined data/lang_refined_rescore \
     data/miami/test ${cleaned_dir}/decode ${cleaned_dir}/decode_rescore
fi
