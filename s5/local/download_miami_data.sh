...
# Citation:
# Orion Weller, Matthias Sperber, Telmo Pessoa Pires, Hendra Setiawan,
...
for i in "${audio[@]}"
do
   if [ ! -f "../data/miami/original/audio/$i.mp3" ]; then
    echo "Downloading $i"
    wget -P data/miami/original/audio/ http://bangortalk.bangor.ac.uk/$i.mp3
   fi
done
...
for i in "${audio[@]}"
do
   if [ ! -f "data/miami/original/audio/$i.wav" ]; then
    echo "converting mp3 to wav $i"
    ffmpeg -i data/miami/original/audio/$i.mp3 -acodec pcm_s16le -ac 1 -ar 16000 data/miami/original/audio/$i.wav
   fi
done
